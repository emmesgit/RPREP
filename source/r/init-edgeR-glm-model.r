	...
	for(s in 1:length(spcFlags)) {
		...
		for (v in 1:length(trtFlags)) {
			...
			for(t in 1:length(postb.times)) {
				...
				if ((glm.model.paired==1 & length(subid.comp)!=0) | (glm.model.paired==0 & length(subid.base)+length(subid.post)!=0)) {
				
					## get ids for treatment time - if paired, filter for subjects that have both pre and post treatment data
					if(glm.model.paired==1) {
						...
						id.time = mta[mta$trt==trtFlag & mta$time %in% c(b.times,time) & !mta$samid %in% outliers,'samid'];
					}
					...
					if(glm.model.paired==1) {
						...
						contrast=c(-1,1);
						dge.lab.time.lrt = glmLRT(dge.lab.time.fit,contrast=contrast);
					}
					
					## get fold change and adjusted p_values
					dge.lab.time.all = topTags(dge.lab.time.lrt,adjust.method="BH", sort.by="logFC",n=NULL);
