...
## Subset to selected assay (defaults to ribosomal profiling)
metadata$assay[is.na(metadata$assay)] = 'ribosomal_profiling'
metadata = metadata[metadata$assay == assay, ]

if (nrow(metadata) == 0) {
    cat('No samples were found for the assay specified on the command line:',assay,'\n')
    q(status=25,save='no')
}
...
if(ensembl.test==F) {
    cat(paste('[parse-configuration]: The specified Ensembl version is invalid:',ensembl.version,'\n'))
    q(status=25,save='no')
}
...
## Append and create assay subdirectory
pre.dir = paste(pre.dir, assay, sep='/')
if(!dir.exists(pre.dir)) {
    system(paste('mkdir',pre.dir))
}

if(!dir.exists(paste(pre.dir,'stage1',sep='/'))) {
    system(paste('mkdir',paste(pre.dir,'stage1',sep='/')))
}
if(!dir.exists(paste(pre.dir,'stage2',sep='/'))) {
    system(paste('mkdir',paste(pre.dir,'stage2',sep='/')))
}
if(!dir.exists(paste(pre.dir,'stage2/genome',sep='/'))) {
    system(paste('mkdir',paste(pre.dir,'stage2/genome',sep='/')))
}
if(!dir.exists(paste(pre.dir,'stage2/annot',sep='/'))) {
    system(paste('mkdir',paste(pre.dir,'stage2/annot',sep='/')))
}
...
## TODO: Append and create assay data subdirectory, set data.dir to the newly created dir
dta.dir = paste(results.dir,'data',assay, sep='/')
...
## Append and create assay analysis subdirectory, set analysis.dir to the newly created dir
if(!dir.exists(paste(results.dir,'analysis',sep='/'))) {
    system(paste('mkdir',paste(results.dir,'analysis',sep='/')))
}

analysis.dir = paste(results.dir,'analysis',assay, sep='/')
...
## Just one report directory for both assays
...
gtf.file = paste(pre.dir,'/stage2/annot/Homo_sapiens.ensembl.version',ensembl.version,'.chr.gtf',sep='')
if(!file.exists(gtf.file)) {
    cat('Downloading GTF annotations from Ensembl FTP site\n')
    system(paste('bash ',source.dir,'/shell/download-ensembl-gtf.sh ',ensembl.version,' ',dirname(gtf.file),sep=''))
}
...
genome.file = paste(pre.dir,'/stage2/genome/Homo_sapiens.ensembl.version',ensembl.version,'.genome.fa',sep='')
if(!file.exists(genome.file)) {
    cat('Downloading Genome annotations from Ensembl FTP site\n')
    system(paste('bash ', source.dir,'/shell/download-ensembl-genome.sh ',ensembl.version,' ',dirname(genome.file),sep=''))
}
...
workflow.config = rbind(workflow.config,c('masking_sequences',masking.seqs))
...
tmp.dta = c(pre.dir, analysis.config[analysis.config$Name=='report_dir','Value'], paste(pre.dir,'preprocess_config.yaml',sep='/'),paste(pre.dir,'sample_metadata.csv',sep='/'), assay, source.dir)
