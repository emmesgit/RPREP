...
libs = c('edgeR','biomaRt','goseq','xtable','MASS','gplots',
         ...
         'gtools','stringr','plyr','grid','gridExtra','UpSetR','Cairo','doParallel');
         
## load libraries using the shared R library
suppressPackageStartupMessages(lapply(libs, require, character.only=T))

## reset variables
rm(list= ls(all=TRUE)[!ls(all=TRUE) %in% 'reportAssayFlag']); 
...
dir.dta = read.csv('../rseqrep/dir.csv',header=F,sep=',',stringsAsFactors=F)

bas.dir = dir.dta[2,1]
pre.dir = dir.dta[1,1]
assay   = dir.dta[5,1]
src.dir = paste0(dir.dta[6,1], '/rseqrep/source')

## We need to be able to specify the correct input directories during reporting
if (exists('reportAssayFlag')) assay = reportAssayFlag

## specify main directories
res.dir  = paste0(bas.dir,'/analysis/',assay)
dta.dir  = paste0(bas.dir,'/data/',assay)

fig.dir  = paste0(bas.dir,'/report/figures')
tbl.dir  = paste0(bas.dir,'/report/tables')
...
## Include sentence based on whether adapters were trimmed 
cutadapt.version = paste0('(Version', system('cutadapt --version', intern=T), ')')
adapter.sentence = '. '
if (!all(is.na(mta$threep_adapter_seq) && !all(is.na(mta$fivep_adapter_seq)))) {
    adapter.sentence = paste0(adapter.sentence, "Next, 3' and 5' adapter sequences were trimmed from reads using \\textit{Cutadapt} ",cutadapt.version,'.')
} else if (!all(is.na(mta$threep_adapter_seq) && any(is.na(mta$fivep_adapter_seq)))) {
    adapter.sentence = paste0(adapter.sentence, "Next, 3' adapter sequences were trimmed from reads using \\textit{Cutadapt} (Version ",cutadapt.version,'.')
} else if (any(is.na(mta$threep_adapter_seq) && !all(is.na(mta$fivep_adapter_seq)))) {
    adapter.sentence = paste0(adapter.sentence, "Next, 5' adapter sequences were trimmed from reads using \\textit{Cutadapt} (Version ",cutadapt.version,'.')
}

bowtie2.version = system("bowtie2 --version | head -n1 | cut -d' ' -f3", intern=T)
fastx.version   = system("fastq_quality_filter -h | tail -n +2 | head -1 | cut -f5 -d' '", intern=T)
hisat2.version  = system("hisat2 --version | head -n1 | cut -d' ' -f3", intern=T)
...
## assays
assayFlags = c('rna_seq', 'ribosomal_profiling')
assayLabls = c('RNA-Seq', 'Ribosomal Profiling')
...
write.tbl = function(x,rownames=F) {
    ...
    xtable.out.file = paste(tbl.dir,'/table_A',tbl.no,'.tab',sep='');
    if(rownames==F) {
...
opts_chunk$set(
        ...
        error          = TRUE,        # Continue after error
