...
if (file.exists(sets.infile)) {
    ...
    ## Add lengths derived from featurecounts to gene annotations
    gen.len.file  = paste0(dta.dir,'/gene_lengths.tab')
    gen.len       = read.table(gen.len.file, h=T, stringsAsFactors=F)
    gen$sum_exon_length = gen.len$Length[match(gen$gene_id, gen.len$Geneid)]
            ...
            for (v in 1:length(trtFlags)) {
                ...
                for(t in 1:length(postb.times)) {
                    ...
                    if(file.exists(in.file.sdeg)) {
                        ...
                        pwf = nullp(DEgenes=gen.bin, genome=NA, id=NA, bias.data=gen$sum_exon_length, plot.fit=FALSE);
