...
<<rseq_variables,echo = FALSE, results= 'hide', message = FALSE>>=
...
%
%---------------------------------------------------------------------
<<rseq_qc_boxplots,fig.height=7.5,fig.cap=paste('Boxplots of human reference genome alignment ',bpLabel,' statistics (RNA-Seq, All specimen types).',sep='')>>=
reportAssayFlag = 'rna_seq'
...
%
%---------------------------------------------------------------------
<<rseq_star_plots,fig.cap=paste('Starplots of human reference genome alignment statistics (RNA-Seq, ',spcLabls,').',sep='')>>=
reportAssayFlag = 'rna_seq'
...
%
%---------------------------------------------------------------------
<<rseq_tmm_box_plots,fig.cap=paste('Boxplots of \\logtwo counts per million ',rep(c('before','after'),each=length(times.knitr.nrm.lab)),' TMM normalization ',rep(times.knitr.nrm.lab,2),'(RNA-Seq, All specimen types).',sep='')>>=
reportAssayFlag = 'rna_seq'
source(paste(src.dir,'/r/00-qc-normalization/tmm-box-plots.r',sep=''));
@

<<rseq_tmm_cdf_plots,fig.width=6,fig.height=7.4,out.width="1.0\\textwidth",fig.cap=paste('Empirical cumulative distribution function plots of \\logtwo counts per million before and after TMM normalization ',times.knitr.nrm.lab,'(RNA-Seq, All specimen types).',sep='')>>=
reportAssayFlag = 'rna_seq'
...
%
%---------------------------------------------------------------------
<<rseq_gbc_plot,fig.width=9,fig.height=6,out.width="1.0\\textwidth",fig.cap=paste('Gene body coverage plots (RNA-Seq).',sep='')>>=
reportAssayFlag = 'rna_seq'
