...
%
%---------------------------------------------------------------------
<<rseq_pca_mds_plots,fig.width=5,fig.height=9,out.height="0.9\\textheight",fig.cap=paste('PCA and non-metric multidimensional scaling biplots (RNA-Seq, ',spcLablsBias,'). PCA biplots with bivariate 95\\% confidence ellipses for standardized variables are shown at the top. Non-metric MDS results for standardized variables and pairwise differences based on Euclidean distance are shown in the middle.  Non-metric MDS results for original variables based on 1-Spearman correlation distance are shown at the bottom. Labels for the four most outlying samples per laboratory based on maximum Mahalanobis distance are shown. Strong outliers are highlighted in blue.',sep='')>>=
