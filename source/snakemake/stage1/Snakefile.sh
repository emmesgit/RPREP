...
rule get_file:
    ...
    run:
        ...
        getfile.getFile(in_file=in_file[0], out_file=output[0], aws_prog=AWS, fastq_dump_prog=FASTQDUMP, openssl_prog=OPENSSL, pw=DECRYPT_PASS)
...
rule fastqc: 
    ...
    run:
        ...
        if DOENCRYPT: output = utils.encryptFile(file=utils.toList([output[0],output[1]]), openssl=OPENSSL, password=ENCRYPT_PASS)
...
rule raw_distribution:
    ...
    run:
        ...
        if DOENCRYPT: output = utils.encryptFile(file=utils.toList(output), openssl=OPENSSL, password=ENCRYPT_PASS)
        if DOARCHIVE: putfile.upload(file=utils.toList(output), destination=ARCHIVE, cloud='aws', prog=AWS)        
