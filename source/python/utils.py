...
def decryptFile(in_file, openssl, password):
    res = []
    for f in in_file:
        cmd = openssl+' aes-256-cbc -md md5 -d -pass pass:'+password+' < '+f+' > ' + os.getcwd() + '/' +os.path.basename(f)[:-4]
        #subprocess.run(cmd, shell=True, check=True)
...
def encryptFile(file, openssl, password):
    res = []
    
    for f in file:
        cmd = openssl+' aes-256-cbc -md md5 -pass pass:'+password+' < '+f+' > ' + f + '.enc'
        #subprocess.run(cmd, shell=True, check=True)
        logging_call(cmd, shell=True)
        res.append(f + '.enc')
    return(res)
